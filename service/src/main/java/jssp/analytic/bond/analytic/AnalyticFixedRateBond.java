...
public class AnalyticFixedRateBond extends AnalyticCouponBond {
    ...
    public AnalyticFixedRateBond withSettlementDate(Date newSettlementDate, DayCounter newYieldDayCounter, Compounding newCompounding) {
        return new AnalyticFixedRateBond(accrualStartDate, maturityDate, firstCouponDate, redemption, couponRate, couponFrequency,
    ...
    public AnalyticFixedRateBond withDayCounters(DayCounter newAccrInterestDayCounter, DayCounter newYieldDayCounter) {
        return new AnalyticFixedRateBond(accrualStartDate, maturityDate, firstCouponDate, redemption, couponRate, couponFrequency,
    ...
    public AnalyticFixedRateBond withOptionExecution(Date executionDate, double strike, DayCounter newYieldDayCounter, Compounding newCompounding) {
        return new AnalyticFixedRateBond(accrualStartDate, executionDate, firstCouponDate, strike, couponRate, couponFrequency,
    ...
    private Bond createBond(DayCounter dayCounter, Schedule bondSchedule) { //TODO
//        if (bondRedemption.redemptionSchedule.isEmpty()) {
...
//        return CashFlows.accruedAmount(legForYieldCalc(), includeSettlementCashflow, date.add(settlementDays)); //TODO
    ...
    private Leg legOfNewRedemption(BondRedemption newBondRedemption) {
        return new AnalyticFixedRateBond(
                accrualStartDate,
                maturityDate,
                firstCouponDate,
                redemption,
    ...
    private DoubleVector couponVectorOf(Schedule schedule) {
        ...
        for (int i = 0; i < scheduleSize - 1; i++) {
            ...
            final double adjustedCouponRate = compensationSchedule.containsKey(couponIndex)
                    ? couponRate + compensationSchedule.get(couponIndex)
                    : couponRate;
